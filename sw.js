...
`;
  ...
  html=html.replace(
    'function showAuth(msg="",type="info"){',
    'function showAuth(msg="",type="info"){document.body.classList.remove("auth-checking");'
  );
  html=html.replace(
    'function showApp(){',
    'function showApp(){document.body.classList.remove("auth-checking");'
  );
  html=html.replace(
    'function showOnboarding(profile=null,edit=false){',
  ...
  try{
    ...
    if(network && network.ok){
      ...
      if(type.includes('text/html')){
        ...
        return new Response(patchAppHtml(text),{
...
self.addEventListener('fetch',e=>{
  ...
  if(isAppNavigation && !url.searchParams.has('skipSplash')){
    e.respondWith(caches.match('./splash.html').then(r=>r||fetch('./splash.html')));
    return;
  }

  if(isAppNavigation && url.searchParams.has('skipSplash')){
    e.respondWith(appResponse(e.request));
    return;
  }
